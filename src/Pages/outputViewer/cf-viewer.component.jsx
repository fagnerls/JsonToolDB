...
const CreditFacilitiesViewer = (props) =>{
...
const handleOpen = (daID) => {
  if (daID === daId || daId === 0) {
    ...
    //const currentDa = selectDA('Credit Facilities');
    const currentDa = props.decisionAreaList.filter(da => da.decisionAreaName.includes( props.decisionAreaName) )
    var currRowId = currentDa.map( ({decisidaidonAreaId , ...otp}) =>(
        otp.actionList.filter(ac => ac.actionValue.includes('FS_CRFA_LIMIT_NAME'))
    ));
    ...
    return (
        <div>

             {
                ...
                <div>
                {//console.log(otherSectionProps.actionList.filter(ac => ac.actionValue.includes('FS_CRFA_LIMIT_NAME')))
                }
...
export default CreditFacilitiesViewer;
